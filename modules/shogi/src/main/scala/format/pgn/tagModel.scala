...
case class Tags(value: List[Tag]) extends AnyVal {
  ...
  def variant: Option[shogi.variant.Variant] =
    apply(_.Variant).map(_.toLowerCase).flatMap {
      case name => shogi.variant.Variant byName name
    }
  ...
  def fen: Option[format.SFEN] = apply(_.SFEN) map format.SFEN.apply
...
object Tags {
  ...
  // according to http://www.saremba.de/shogigml/standards/pgn/pgn-complete.htm#c8.1.1
