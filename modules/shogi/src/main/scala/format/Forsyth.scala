...
/**
  ...
  * https://github.com/ddugovic/Stockfish/wiki/SFEN-extensions
  * http://scidb.sourceforge.net/help/en/SFEN.html#ThreeCheck
  */
object Forsyth {
  ...
  // only cares about pieces positions on the board (first part of SFEN string)
  ...
  def exportCrazyPocket(board: Board) = {
    val pocketStr = board.crazyData match {
      case Some(shogi.Data(pockets, _)) =>
