...
import Pos._
import format.Uci
import variant.{ Antishogi, Atomic, Crazyhouse, Standard, ThreeCheck }
...
class HashTest extends ChessTest {
  ...
  "Hasher" should {
    ...
    "be consistent in crazyhouse" in {
      // from https://lishogi.org/j4r7XHTB/black
    ...
    "be consistent when king is captured in antishogi" in {
      val fen           = "rnbqkb1r/ppp1pppp/3p1n2/1B6/8/4P3/PPPP1PPP/RNBQK1NR w KQkq - 2 3"
      val situation     = ((format.Forsyth << fen) get) withVariant Antishogi
      ...
      val situationAfter = ((format.Forsyth << fenAfter) get) withVariant Antishogi
      val hashAfter      = hash(situationAfter)
    ...
    "prod 5 Three-Check games accumulate hash" in {
      ...
      def runOne(moves: List[String]) =
        Replay.gameMoveWhileValid(moves, format.Forsyth.initial, shogi.variant.ThreeCheck)
