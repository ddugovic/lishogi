...
trait ChessTest extends Specification with ValidationMatchers {
  ...
  implicit def stringToBoardBuilder(str: String) =
    new {
      ...
      def shogi960: Board = makeBoard(str, shogi.variant.Chess960)

      def kingOfTheHill: Board = makeBoard(str, shogi.variant.KingOfTheHill)

      def threeCheck: Board = makeBoard(str, shogi.variant.ThreeCheck)
    }
  ...
  def fenToGame(positionString: String, variant: Variant) = {
    ...
    } toValid "Could not construct situation from SFEN" map {
      case (color, board) =>
        Game(variant).copy(
