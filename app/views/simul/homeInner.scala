          ...
          tbody(
            pendings.map { sim =>
              tr(cls := "scheduled")(
                simTd(sim),
                simHost(sim),
                td(cls := "players text", dataIcon := Icons.person)(sim.applicants.size),
              )
            },
          ),
        ),
          ...
          frag(
            ...
            starteds.map { sim =>
              tr(
                simTd(sim),
                simHost(sim),
                td(cls := "players text", dataIcon := Icons.person)(sim.pairings.size),
              )
            },
          )
        ),
  ...
  private def simTd(sim: lila.simul.Simul)(implicit lang: Lang) =
    td(cls := "header")(
      a(href := routes.Simul.show(sim.id))(
        span(cls := "name")(trans.xSimul(sim.name)),
        bits.setup(sim),
      ),
    )
